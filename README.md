...
| 7  | **16MB transaction payload limit (row AND multi-statement)**          | [Demo 6](#demo-6-large-payload--16mb-transaction-limit)                                         | Limit applies to TOTAL txn payload. 25 stmts x 500KB each = 12.5MB, close to the limit.                     |
| 7a | **`split failed while applying backpressure to Put` errors**          | [Demo 6](#demo-6-large-payload--16mb-transaction-limit)                                         | Large rows/txns block range splits; Raft stalls. Keep rows <1MB, txns <4MB.                                 |
